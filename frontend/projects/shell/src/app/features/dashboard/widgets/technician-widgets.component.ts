          ...
          @for (i of interventions().slice(0, 6); track i.id) {
            <li class="intervention-item">
              <div class="inter-icon">
                {{ i.statut === 'en_cours' ? '🔄' : i.statut === 'termine' ? '✅' : '📋' }}
              </div>
        ...
        <section class="card alert-section">
          ...
          @for (a of alerts(); track $index) {
            <div class="alert-row">
              <span>⚠️</span>
              <span
                >{{ a.type_intervention ?? 'Maintenance requise' }} — Véhicule
                {{ a.vehicule_id ?? 'N/A' }}</span
              >
            </div>
          }
        </section>
...
export class TechnicianWidgetsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    const query = {
      query: `
      query {
        interventions { id vehicule_id statut type_intervention date_planifiee }
        preventiveAlerts(days: 7) { count alerts { id vehicule_id type_intervention } }
      }
    `,
    };

    this.http
      .post<any>(graphqlUrl, query)
      .pipe(catchError(() => of({ data: { interventions: [], preventiveAlerts: { alerts: [] } } })))
      .subscribe((response) => {
        const data = response.data;

        this.interventions.set(data.interventions || []);
        this.alerts.set(data.preventiveAlerts?.alerts || []);

        this.interventionsTerminees.set(
          (data.interventions || []).filter((x: any) => x.statut === 'termine').length,
        );
