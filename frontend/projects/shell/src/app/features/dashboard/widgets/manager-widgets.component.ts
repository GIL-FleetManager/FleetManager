...
@Component({
    ...
    @if (isLoading()) {
      ...
      <section class="card mt">
        ...
        <table class="data-table">
          <thead><tr><th>Nom</th><th>Prénom</th><th>Téléphone</th><th>Statut Permis</th></tr></thead>
          <tbody>
            @for (c of conductors().slice(0, 6); track c.id) {
              <tr>
                <td>{{ c.nom }}</td><td>{{ c.prenom }}</td>
                <td>{{ c.telephone }}</td>
                <td><span class="badge badge-info">{{ c.statut ?? 'actif' }}</span></td>
              </tr>
            } @empty {
              <tr><td colspan="4" class="empty">Aucun conducteur</td></tr>
            }
          </tbody>
        </table>
      </section>
  ...
  styles: [`
    ...
    .spinner { width:36px;height:36px;border:3px solid #e0e0e0;border-top-color:#7209b7;border-radius:50%;animation:spin 0.8s linear infinite; }
    @keyframes spin{to{transform:rotate(360deg)}}
    .widget-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:1.2rem;margin-bottom:1.5rem}
    ...
    .blue .stat-value{color:#7209b7} .orange .stat-value{color:#f48c06} .green .stat-value{color:#06d6a0}
    .card{background:#fff;border-radius:14px;padding:1.4rem;box-shadow:0 2px 10px rgba(0,0,0,.06)}
    .mt{margin-top:0}
    h3{font-size:1rem;font-weight:600;margin-bottom:1rem}
    .data-table{width:100%;border-collapse:collapse;font-size:.875rem}
    .data-table th{text-align:left;padding:8px 12px;color:#888;font-weight:600;font-size:.72rem;text-transform:uppercase;border-bottom:1px solid #f0f0f0}
    ...
    .badge-info{background:#e8f0fe;color:#4361ee}
  `]
})
export class ManagerWidgetsComponent implements OnInit {
  ...
  ngOnInit(): void {
    forkJoin({
      ...
      this.vehiclesDisponibles.set(v.filter((x: any) => x.statut === 'disponible' || x.statut === 'actif').length);
